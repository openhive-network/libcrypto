...
sjcl.codec.hive = {
  ...
  keysFromPassword: function(account, password) {
    ...
    for (var i = 0; i < sjcl.codec.hive.ROLES.length; i++) {
      var role = sjcl.codec.hive.ROLES[i];
  ...
  signRecoverably: function(sec, hash, paranoia, fixedKForTesting) {
     ...
     * its X coordinate).
     *
     * note that in the Hive blockchain, all public keys are canonically compressed and therefore the
  ...
  recoverPublicKey: function(hash, sig) {
    ...
    for (var j = 0; j <= 1; j++) {
      ...
      var y = sjcl.codec.hive._yFromX(x, hasOddParity);
  ...
  serializePublicKey: function(key, net) {
    net = net || sjcl.codec.hive.MAINNET;
  ...
  deserializePublicKey: function(pubKey, net) {
    ...
    var y = sjcl.codec.hive._yFromX(x, isOdd);
  ...
  deserializePrivateKey: function(wif, header) {
    header = header || sjcl.codec.hive.MAINNET.privHeader;
    var curve = sjcl.ecc.curves.k256;
  ...
  _yFromX: function(x, shouldBeOdd) {
    var CURVE = sjcl.ecc.curves.k256;
    var PIDENT = sjcl.codec.hive._getPident();
  ...
  _getPident: function() {
    if (!sjcl.codec.hive.PIDENT) {
      sjcl.codec.hive.PIDENT = sjcl.ecc.curves.k256.field.modulus
        .add(1)
        .normalize()
        .halveM()
        .halveM()
