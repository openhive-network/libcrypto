...
(function(root, factory) {
  ...
  function PrivateKey(priv, pub) {
    ...
    this.sign = function(hash) {
      return fromBits(sjcl.codec.hive.signRecoverably(priv, toBits(hash)));
    };
  }

  PrivateKey.from = function(wif, header) {
    return new PrivateKey(
      sjcl.codec.hive.deserializePrivateKey(wif, header)
    );
  };
  ...
  PublicKey.from = function(str) {
    return new PublicKey(sjcl.codec.hive.deserializePublicKey(str));
  };

  PublicKey.recover = function(hash, sig) {
    return new PublicKey(
      sjcl.codec.hive.recoverPublicKey(toBits(hash), toBits(sig))
    );
  };

  PublicKey.prototype = {
    toString: function() {
      return sjcl.codec.hive.serializePublicKey(this._p);
    },
  ...
  function serializePair(k) {
    return {
      private: sjcl.codec.hive.serializePrivateKey(k.sec),
      public: sjcl.codec.hive.serializePublicKey(k.pub)
    };
  }
